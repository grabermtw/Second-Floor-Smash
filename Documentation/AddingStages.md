...
3. Add a BoxCollider2D to the GameplayManager object and check the box titled `Is Trigger` to make it a trigger. This will delineate the boundaries of the stage: if a player exits this trigger, the player is KO'd. Resize this box so that it can serve as an appropriate boundary.
